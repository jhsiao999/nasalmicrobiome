...
saveRDS(res, file = "../output/gom_seeds.Rmd/res.rds")
```


```{r, eval = T}
res <- readRDS(file="../output/gom_seeds.Rmd/res.rds")
```
...
```{r, eval=F}
saveRDS(res[[80]], file = "res_best_seed_409.rds")
```
...
```{r, eval=T}
res_best <- readRDS(file = "res_best_seed_409.rds")
```
