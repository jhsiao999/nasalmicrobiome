...
```{r, eval = F}
fits <- FitGoM(t(counts), K=c(2:20), tol=0.00001)
saveRDS(fits, file = "output/chossing-number-of-clusters.Rmd/gomfits.rds")
```
...
bf_species <- do.call(rbind, lapply(1:length(fits), function(i) {
  data.frame(no_cluster = c(2:20)[i], BF = fits[[i]]$BF)
}) )
